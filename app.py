...
corrige errores de lectura de fechas (KeyError) y concentra la atención 
exclusivamente en el P&G, Costos y Eficiencia de Lotes/Alimento.
"""
...
def load_and_process_data(file_source):
    ...
    # --- EXTRACCIÓN TÉCNICA Y CORRECCIÓN DE FECHAS (ETL) ---
    def extract_tech(sheet):
        if sheet in xls.sheet_names:
            d = pd.read_excel(xls, sheet_name=sheet)
            d.columns = d.columns.astype(str).str.strip()
            
            if 'Lote' in d.columns: 
                d['Lote_str'] = d['Lote'].apply(clean_lote)
            
            # Corrección del KeyError: Evaluar FECHA primero (Aplica para BD PN HF RAZA)
            if 'FECHA' in d.columns:
                d['FECHA'] = pd.to_datetime(d['FECHA'], errors='coerce')
                d['Anio'] = d['FECHA'].dt.year.fillna(2026).astype(int)
                d['Mes_Num'] = d['FECHA'].dt.month.fillna(1).astype(int)
            else:
    ...
    if "BASE ZCO001" not in xls.sheet_names: return pd.DataFrame(), [], pd.DataFrame(), pd.DataFrame()
    ...
    return df_res, rubros, df_raw, df_raza
...
try:
    df, rubros_items, df_raw, df_raza = load_and_process_data(file_to_load)
    if df.empty: st.stop()
...
# =============================================================================
# MÓDULOS DEL DASHBOARD (STORYTELLING + DATA FINANCIERA)
# =============================================================================
...
if menu == "1. Producción Global y Mix Genético":
    ...
    # Prevención del KeyError asegurando que 'Periodo' existe antes del cruce
    if not df_raza.empty and 'Periodo' in df_raza.columns and p_actual in df_raza['Periodo'].values:
    ...
    with col_grafico:
        st.subheader("📊 Ranking de Ineficiencia")
