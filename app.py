...
def load_and_process_data(file_source):
    ...
    # APROVECHAMIENTO: La clave de la reconciliación
...
def generar_diagnostico_costos(df_impactos, var_total, p_actual, p_base):
    ...
    recomendacion = RECOMENDACIONES_RUBRO.get(
        ...
        f"desviación desfavorable. Requiere revisión inmediata de presupuestos.",
    )
    return (
        f'<div class="alert-box">🚨 <b>Alerta de Desviación Financiera '
...
# =============================================================================
# 7. PÁGINA 3 — COSTO HUEVO FÉRTIL (VIF, EVOLUTIVA & SIMULADOR)
# =============================================================================
    ...
    if pd.isna(df_b["Costo Huevo Fértil"]) or pd.isna(df_a["Costo Huevo Fértil"]):
        st.error("Alguno de los dos períodos no tiene huevos fértiles registrados; "
...
# =============================================================================
    ...
    # Costos Lote (Aplica la misma lógica que el consolidado para asegurar el match)
    ...
    df_m = pd.DataFrame({"Costo Base": c_lote, "Aprovechamientos": a_lote, "Huevos Fértiles": h_lote}).fillna(0)
    ...
    <div class="alert-box">
        ...
        <i>¿Por qué sucede esto?</i> Usualmente un volumen muy bajo de producción provoca que los costos fijos (depreciación gallina/M.O.) se dividan entre muy pocas unidades. <b>Evaluar descarte por curva de postura.</b><br><br>
        ✅ El <b>Lote {lote_eficiente['Lote']}</b> sostiene la rentabilidad con un costo óptimo de <b>${lote_eficiente['Costo Unitario']:,.1f} COP/HF</b>.
    </div>
    """, unsafe_allow_html=True)
...
# =============================================================================
    ...
    <div class="insight-box">
        💡 <b>Sensibilidad Financiera:</b> El alimento pondera la mayor parte del costo total operativo. Monitorear las fluctuaciones del precio del kilogramo y la conversión alimenticia (gramos consumidos por huevo) es el pilar de la rentabilidad avícola.
    </div>
    """, unsafe_allow_html=True)
